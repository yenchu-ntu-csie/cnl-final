...
```
 ...
 自主/路由層 (p2p_node) autonomous 群組(--auto, 平行 capability scan)
                       S4 知識路由(--route): ROUTE_QUERY/ROUTE_ANSWER 多跳 + 聲望 targeted + provenance
 加密層 (e2ee)        X25519 + ChaCha20-Poly1305(AEAD)，每訊息臨時金鑰，metadata 綁 AAD
 中繼 (relay_server)  依公鑰轉發、看不到明文；離線存轉(queue + 上線補投)
 信任 (agents)        白名單(fail-closed) + per-peer tier + per-topic reputation
```
...
4. **安全性（test_security 6 項全過）**：空白名單 fail-closed 拒收、未授權拒收、common 經 S4 也拿不到 personal（tier ACL 不被路由繞過）。
5. **韌性（test_storeforward）**：peer 離線 → relay 暫存、上線補投，訊息不丟。
